...
> 6. **`OptimizerOptions` struct replaced 5/6-arg positional options.** The v1 trait had four args. Each follow-up flag (`--max-colors`, `--png-optimization-level`) widened the trait signature and required a 5-impl + N-test-site churn. The trait was refactored to take `&OptimizerOptions` (Debug + Clone + Default) so the next flag is a one-field change. See commit `780914c`.
> 7. **`-v` / `--verbose` and `--summary-only` shipped as follow-up observability flags.** The v1 plan did not call for per-step trace output or summary-only output. Both are purely additive: `--verbose` adds trace lines to stderr (PNG only — JPEG/GIF/WebP/SVG have no interesting step breakdown), and `--summary-only` suppresses per-file result lines on stdout. Implementation: `verbose: bool` added to `OptimizerOptions`; `summary_only: bool` added to `Reporter`. No new field on the `OptimizationResult` return type, no side-channel — the optimizer just `eprintln!`s when verbose.
