...
fn jpeg_round_trip() {
    ...
    let output = optimizer
        .optimize(&input, None, false, false, None)
...
fn jpeg_quality_affects_output_size() {
    ...
    let input = make_jpeg();
    let high = optimizer
        .optimize(&input, Some(95), false, false, None)
        .expect("q=95");
    let low = optimizer
        .optimize(&input, Some(20), false, false, None)
        .expect("q=20");
    assert!(
        low.len() < high.len(),
...
fn png_lossy_smaller_than_lossless() {
    ...
    // Use the synthetic photo fixture in tests/example01.png — a 2.3
    // MB RGB PNG generated by `cargo run --example gen-fixtures`.
    // The image is procedurally generated to behave like a natural
    // photo: smooth color regions with low-amplitude noise, so the
    // palette quantizer can compress it well.
    ...
    let lossless = optimizer
        ...
        .expect("lossless");
    let lossy = optimizer
        .optimize(&input, None, true, false, None)
        .expect("lossy");
    assert!(
    ...
    let lossy_no_zopfli = optimizer
        .optimize(&input, None, true, true, None)
